...
async function populateFacultiesAndDepartments() {
  ...
  facultySelect.innerHTML = `<option value="">Select Faculty</option>` + facultiesCache.map(f =>
    `<option value="${f._id}">${f.name}</option>`
  ).join('');
  // Listen for faculty changes to reload departments
  facultySelect.onchange = function() {
    const selectedFaculty = facultySelect.value;
    const deptSelect = document.getElementById("editDepartment");
    const relatedDepts = departmentsCache.filter(d => d.faculty === selectedFaculty);
    deptSelect.innerHTML = `<option value="">Select Department</option>` + relatedDepts.map(d =>
      `<option value="${d._id}">${d.name}</option>`
    ).join('');
  };
  // On department load, populate for current faculty
  if (student.facultyId) {
    facultySelect.value = student.facultyId;
    facultySelect.onchange();
    if (student.departmentId) {
      document.getElementById("editDepartment").value = student.departmentId;
    }
  }
}
...
async function fetchProfile() {
  ...
  // Fac/Dept loaded separately
  await populateFacultiesAndDepartments();
}

// --- Save Profile Button with Loader ---
document.getElementById("saveProfileBtn").onclick = async function() {
  document.getElementById("profileEditLoader").style.display = "block";
  ...
  const payload = { username, email, phone, faculty: facultyId, department: departmentId, level };
  ...
  try {
    ...
    document.getElementById("profileEditLoader").style.display = "none";
    if (!resp.ok) {
      const data = await resp.json();
    ...
    await fetchProfile(); // Refresh fields
    await fetchAvailableTests(); // In case faculty/department changed
  } catch (e) {
    document.getElementById("profileEditLoader").style.display = "none";
