...
function getKey() {
  const keyB64 = process.env.FACE_DESCRIPTOR_KEY;
  if (!keyB64) throw new Error("FACE_DESCRIPTOR_KEY not set");
  const key = Buffer.from(keyB64, "base64");
  if (key.length !== 32) throw new Error("FACE_DESCRIPTOR_KEY must be 32 bytes (base64)");
  return key;
}
