...
export async function descriptorFromBuffer(buffer) {
  try {
    ...
    let img;
    try { img = await loadImage(buffer); } catch (e) { return null; }
    if (!img || !img.width || !img.height) return null;
    const canv = createCanvas(img.width, img.height);
    const ctx = canv.getContext("2d");
    ctx.drawImage(img, 0, 0, img.width, img.height);
    if (canv.width === 0 || canv.height === 0) return null;
    const det = await faceapi
      .detectSingleFace(canv, new faceapi.TinyFaceDetectorOptions())
      .withFaceLandmarks()
      .withFaceDescriptor();
    if (!det || !det.descriptor) return null;
    return Array.from(det.descriptor).map(Number);
  } catch (err) {
    console.error("descriptorFromBuffer unexpected error:", err);
    return null;
  }
}
