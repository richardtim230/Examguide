...
const RCCG_UserSchema = new Schema(
  {
    ...
    // Church Departments & Ministries (Updated to Strings)
    departments: [
      {
        type: String,
        trim: true,
    ...
    // Attendance & Events (Updated event field to generic ObjectId/String)
    attendanceRecords: [
      {
        _id: {
          type: Schema.Types.ObjectId,
    ...
    // Events Registered (Updated to generic Array of ObjectIds)
    registeredEvents: [
      {
        type: Schema.Types.ObjectId,
      },
    ...
    // Saved Items (Updated to plain ObjectIds)
    savedSermons: [
      {
        type: Schema.Types.ObjectId,
      },
...
RCCG_UserSchema.pre("save", async function (next) {
  try {
    ...
    if (!this.membershipNumber) {
      ...
      this.membershipNumber = `RCCG-${new Date().getFullYear()}-${(count + 1)
        .toString()
        .padStart(5, "0")}`;
    }
