...
// Note: I added 'uploadToSupabase' here. Change this to match your actual Supabase upload function's name.
import { createResource, uploadEditorImage, deleteSupabaseFile, uploadToSupabase } from "../controllers/resourceController.js";
...
// Memory Storage used for direct stream/buffer handoff to Supabase
...
router.get("/user/library", authenticate, async (req, res) => {
  try {
    ...
    const [
      ...
      Resources.find({ uploader: userId })
      ...
      Progress.find({ user: userId })
        .populate({
          path: "resource",
          populate: { path: "uploader", select: "fullname profilePic username faculty department level" }
        })
        .populate("chapter", "chapterNumber title")
        .sort({ updatedAt: -1 })
        .lean(),

      ReadAccess.find({ user: userId })
        .populate({
          path: "resource",
          populate: { path: "uploader", select: "fullname profilePic username faculty department level" }
        })
    ...
    const uploadedBooks = uploadedResources.filter(r => r.resourceType === "textbook");
    const uploadedNotes = uploadedResources.filter(r => r.resourceType === "notebook");
    const progressMap = new Map();

    for (const progress of readingProgress) {
      if (!progress.resource?._id) continue;
      progressMap.set(String(progress.resource._id), {
        chapter: progress.chapter ? { _id: progress.chapter._id, chapterNumber: progress.chapter.chapterNumber, title: progress.chapter.title } : null,
    ...
    const addProgress = resource => {
      ...
      return {
        ...resource,
        progress: progress || { chapter: null, page: 1, updatedAt: null, progressPercent: 0 }
      };
    };
    ...
    res.json({
      ...
      library: {
        books: uploadedBooks.map(addProgress),
        notes: uploadedNotes.map(addProgress)
      },
      history: readingProgress.filter(item => item.resource).map(item => ({
        ...item.resource,
        progress: {
          chapter: item.chapter ? { _id: item.chapter._id, chapterNumber: item.chapter.chapterNumber, title: item.chapter.title } : null,
          page: item.page || 1,
          progressPercent: item.progressPercent || 0,
          updatedAt: item.updatedAt
        }
      })),
      accessed: accessedResources.filter(item => item.resource).map(item => ({
        ...item.resource,
        access: { paidAt: item.paidAt || item.createdAt || null },
        progress: progressMap.get(String(item.resource._id)) || { chapter: null, page: 1, updatedAt: null, progressPercent: 0 }
      })),
      bookmarks: bookmarks.filter(item => item.resource).map(item => ({
        ...item.resource,
    ...
    res.status(500).json({ success: false, error: "Failed to fetch user library" });
  }
});
...
router.get("/users/me/library", authenticate, async (req, res) => {
  try {
    const userId = req.user._id;
    const user = await User.findById(userId).select("fullname username profilePic faculty department level").lean();

    if (!user) {
      return res.status(404).json({ success: false, error: "User not found" });
    }

    const uploadedResources = await Resources.find({ uploader: userId })
      .sort({ createdAt: -1 })
      .populate("uploader", "fullname username profilePic")
      .lean();

    const books = uploadedResources.filter(r => r.resourceType === "textbook");
    const notes = uploadedResources.filter(r => r.resourceType === "notebook");
    const resourceIds = uploadedResources.map(r => r._id);

    const progresses = await Progress.find({ user: userId, resource: { $in: resourceIds } }).sort({ updatedAt: -1 }).lean();
    const progressMap = new Map(progresses.map(p => [String(p.resource), p]));

    const mapWithProgress = items => items.map(item => {
      ...
      return {
        ...
        progress: progress ? {
          ...
          totalPages: progress.totalPages || item.pages || 0,
          updatedAt: progress.updatedAt,
          lastOpenedAt: progress.lastOpenedAt || progress.updatedAt
        } : { progressPercent: 0, currentPage: 0, totalPages: item.pages || 0, updatedAt: null, lastOpenedAt: null }
      };
    });

    const accessedRecords = await ReadAccess.find({ user: userId })
      .sort({ paidAt: -1 })
      .populate({ path: "resource", populate: { path: "uploader", select: "fullname username profilePic" } })
      .lean();

    const historyRecords = await Progress.find({ user: userId })
      .sort({ updatedAt: -1 })
      .limit(50)
      .populate({ path: "resource", populate: { path: "uploader", select: "fullname username profilePic" } })
      .lean();

    const totalOpened = await Progress.countDocuments({ user: userId });
    ...
    return res.json({
      ...
      stats: {
        ...
        totalAccessed: accessedRecords.length,
        totalOpened,
        totalBookmarks: 0
      },
      library: {
        books: mapWithProgress(books),
        notes: mapWithProgress(notes)
      },
      accessed: accessedRecords.filter(item => item.resource).map(item => {
        ...
        return {
          ...
          access: { paidAt: item.paidAt, createdAt: item.createdAt },
          progress: progress ? { progressPercent: progress.progressPercent || 0, currentPage: progress.currentPage || 0, totalPages: progress.totalPages || resource.pages || 0, updatedAt: progress.updatedAt, lastOpenedAt: progress.lastOpenedAt || progress.updatedAt } : { progressPercent: 0, currentPage: 0, totalPages: resource.pages || 0, updatedAt: null, lastOpenedAt: null }
        };
      }),
      history: historyRecords.filter(item => item.resource).map(item => {
        const resource = item.resource;
        return {
          _id: resource._id,
          title: resource.title,
          ...
          uploader: resource.uploader,
          coverImage: resource.cover?.url || "",
          progress: { progressPercent: item.progressPercent || 0, currentPage: item.currentPage || 0, totalPages: item.totalPages || resource.pages || 0, updatedAt: item.updatedAt, lastOpenedAt: item.lastOpenedAt || item.updatedAt }
        };
      }),
      bookmarks: []
    });
    ...
    return res.status(500).json({ success: false, error: "Failed to load library" });
  }
});

// ✅ MOVED UP: Bookmarks specific route to prevent route shadowing
...
router.get("/users/:id/library", async (req, res) => {
  try {
    ...
    const [
      uploadedResources, uploadedTotal,
      progressRecords, openedTotal,
      bookmarkRecords, bookmarkTotal
    ] = await Promise.all([
      Resources.find({ uploader: userId }).populate("uploader", "fullname username profilePic faculty department level").sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
      Resources.countDocuments({ uploader: userId }),
      Progress.find({ user: userId }).populate({ path: "resource", populate: { path: "uploader", select: "fullname username profilePic faculty department level" } }).populate("chapter", "chapterNumber title").sort({ updatedAt: -1 }).skip(skip).limit(limit).lean(),
      Progress.countDocuments({ user: userId }),
      Bookmark.find({ user: userId }).populate({ path: "resource", populate: { path: "uploader", select: "fullname username profilePic faculty department level" } }).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    ...
    res.json({
      success: true,
      user: {
        _id: user._id, fullName: user.fullname || "", username: user.username || "", profilePicture: user.profilePic || "",
        ...
        institution: typeof user.institution === "object" ? user.institution?.name || "" : user.institution || "",
        level: user.level || "", bio: user.bio || ""
      },
      stats: { uploaded: uploadedTotal, opened: openedTotal, bookmarked: bookmarkTotal },
      uploaded: { items: uploadedResources, total: uploadedTotal, page, limit, hasMore: skip + uploadedResources.length < uploadedTotal },
      opened: {
        items: progressRecords.filter(item => item.resource).map(item => ({ ...item.resource, readingProgress: { chapter: item.chapter || null, page: item.page || 1, updatedAt: item.updatedAt } })),
        total: openedTotal, page, limit, hasMore: skip + progressRecords.length < openedTotal
      },
      bookmarked: {
        items: bookmarkRecords.filter(item => item.resource).map(item => ({ ...item.resource, bookmarkedAt: item.createdAt })),
        total: bookmarkTotal, page, limit, hasMore: skip + bookmarkRecords.length < bookmarkTotal
      }
    });
  } catch (err) {
    console.error("Fetch user library error:", err);
...
router.get("/users/:id", async (req, res) => {
  try {
    ...
    const user = await User.findById(id).populate("institution", "name").populate("faculty", "name").populate("department", "name").select("fullname profilePic faculty department level bio institution username").lean();
    if (!user) return res.status(404).json({ error: "User not found" });
...
router.post("/users/upload-avatar", authenticate, avatarUpload.single("profilePic"), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: "Please upload an image." });
    const user = await User.findByIdAndUpdate(req.user.id, { profilePic: req.file.publicUrl }, { new: true }).select("-password");
    res.json({ success: true, message: "Profile picture updated successfully.", profilePic: req.file.publicUrl, user });
  } catch (err) {
    console.error(err);
...
router.post("/", authenticate, multerMiddleware([{ name: "mainFile", maxCount: 1 }, { name: "coverFile", maxCount: 1 }]), async (req, res, next) => {
  try {
...
router.get("/", async (req, res) => {
  ...
  if (req.query.uploader) {
    if (!mongoose.Types.ObjectId.isValid(req.query.uploader)) return res.status(400).json({ error: "Invalid uploader id" });
  ...
  if (q) filter.$or = [{ title: new RegExp(q, "i") }, { subtitle: new RegExp(q, "i") }, { authors: new RegExp(q, "i") }, { courseCode: new RegExp(q, "i") }];
  
  const skip = (page - 1) * limit;

  const [items, total] = await Promise.all([
    Resources.find(filter).populate("uploader", "fullname avatar faculty department level").sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    const resource = await Resources.findById(id).populate("uploader", "fullname profilePic faculty department level username").lean();
    if (!resource) return res.status(404).json({ error: "Resource not found" });
    res.json({ success: true, resource });
  } catch (err) { next(err); }
});

router.put("/:id", authenticate, multerMiddleware([{ name: "mainFile", maxCount: 1 }, { name: "coverFile", maxCount: 1 }]), async (req, res, next) => {
    try {
      const id = req.params.id;
      if (!isValidId(id)) {
        await removeFilesSafely([req.file, req.coverFile]);
      ...
      // ✅ FIX: Actually upload the buffer to Supabase instead of discarding it
      if (req.file) {
        const f = req.file;
        
        // This assumes uploadToSupabase accepts a buffer/mimetype/name and returns the URL and IDs.
        // Update this method call depending on what your resourceController exports.
        const uploadedFile = await uploadToSupabase(f.buffer, f.originalname, f.mimetype);
        ...
        const fileObj = {
          ...
          bucket: uploadedFile.bucket || "resources",
          publicId: uploadedFile.publicId || uploadedFile.fileId,
          fileId: uploadedFile.publicId || uploadedFile.fileId,
          uploadedAt: new Date()
        };
      ...
      // ✅ FIX: Same logic applies to the cover coverFile
      if (req.coverFile) {
        const cf = req.coverFile;
        const uploadedCover = await uploadToSupabase(cf.buffer, cf.originalname, cf.mimetype);

        doc.cover = {
          url: uploadedCover.url,
          mimeType: cf.mimetype,
          size: cf.size || cf.buffer.length,
          storageType: "supabase",
          bucket: uploadedCover.bucket || "covers",
          publicId: uploadedCover.publicId || uploadedCover.fileId
        };
      }
...
router.post("/:id/bookmark", authenticate, async (req, res) => {
  try {
    ...
    const bookmark = await Bookmark.create({ resource: resourceId, user: req.user._id });
    res.json({ success: true, bookmarked: true, bookmark });
  } catch (err) {
    if (err.code === 11000) return res.status(409).json({ error: "Resource already bookmarked" });
...
router.get("/:id/bookmark", authenticate, async (req, res) => {
  try {
    ...
    res.json({ success: true, bookmarked: !!bookmark, bookmarkCount });
  } catch (err) {
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/:id/progress", authenticate, async (req, res) => {
  try {
    const resourceId = req.params.id;
    if (!isValidId(resourceId)) return res.status(400).json({ error: "Invalid resource id" });
    const progress = await Progress.findOne({ resource: resourceId, user: req.user._id }).populate("chapter", "chapterNumber title").lean();
...
router.post("/:id/progress", authenticate, async (req, res) => {
  try {
    ...
    if (String(resource.uploader) !== String(req.user._id)) {
      const access = await ReadAccess.findOne({ user: req.user._id, resource: resourceId }).lean();
      if (!access) return res.status(403).json({ error: "You don't have access to this resource." });
    }

    const { chapterId, page } = req.body;
    if (chapterId && !isValidId(chapterId)) return res.status(400).json({ error: "Invalid chapter id" });
    if (chapterId) {
    ...
    const progress = await Progress.findOneAndUpdate(
      ...
      { chapter: chapterId || null, page: Number(page) || 1, updatedAt: new Date() },
      { new: true, upsert: true, setDefaultsOnInsert: true }
    );
    const populated = await Progress.findById(progress._id).populate("chapter", "chapterNumber title").lean();
    res.json({ success: true, progress: populated });
  } catch (err) { res.status(500).json({ error: "Failed to update reading progress" }); }
});
...
router.get("/:id/read-status", authenticate, async (req, res) => {
  try {
    ...
    if (String(resource.uploader) === String(req.user._id)) {
      return res.json({ success: true, hasAccess: true, isOwner: true, paidAt: null });
    }

    const access = await ReadAccess.findOne({ resource: resourceId, user: req.user._id }).lean();
    res.json({ success: true, hasAccess: !!access, isOwner: false, paidAt: access?.paidAt || null });
  } catch (err) { res.status(500).json({ error: "Server error" }); }
});
...
router.get("/:id/read-access", authenticate, async (req, res) => {
  try {
    const { id } = req.params;
    if (!isValidId(id)) return res.status(400).json({ error: "Invalid resource id" });
    const resource = await Resources.findById(id).select("uploader").lean();
    if (!resource) return res.status(404).json({ error: "Resource not found" });

    if (String(resource.uploader) === String(req.user._id)) {
      return res.json({ success: true, hasAccess: true, owner: true, alreadyPaid: false });
    }
    const access = await ReadAccess.findOne({ user: req.user._id, resource: id }).lean();
    res.json({ success: true, hasAccess: !!access, owner: false, alreadyPaid: !!access });
  } catch (err) { res.status(500).json({ error: "Server error" }); }
});
...
router.post("/:id/read", authenticate, async (req, res) => {
  ...
  try {
    ...
    const existingAccess = await ReadAccess.findOne({ user: reader._id, resource: resource._id }).session(session);
    if (existingAccess) {
      await session.commitTransaction();
      return res.json({ success: true, alreadyPaid: true, readerCredits: reader.creditPoints });
    }
    ...
    await User.updateOne({ _id: reader._id }, { $inc: { creditPoints: -10 } }, { session });
    await User.updateOne({ _id: uploader._id }, { $inc: { points: 5 } }, { session });

    await ReadAccess.create([{ user: reader._id, resource: resource._id, paidAt: new Date() }], { session });
    await CreditTransaction.create([{ from: reader._id, to: uploader._id, resource: resource._id, amount: 10, uploaderReward: 5 }], { session });

    await session.commitTransaction();
    res.json({ success: true, alreadyPaid: false, readerCredits: reader.creditPoints });
  } catch (err) {
    await session.abortTransaction();
